...
export async function AppShell({
  ...
  return (
    <div className="min-h-screen bg-[var(--bg)] lg:grid lg:grid-cols-[248px_minmax(0,1fr)]">
      <aside className="border-r border-[var(--line)] bg-white/95 lg:sticky lg:top-0 lg:h-screen lg:overflow-y-auto max-lg:border-r-0 max-lg:border-b">
        <div className="flex h-full flex-col p-4">
          ...
          <div className="mt-4 flex items-center gap-3 rounded-lg border border-slate-200 bg-slate-50 p-3">
            ...
            <div className="min-w-0">
              ...
              <div className="mt-0.5 text-xs font-bold uppercase text-slate-500">{profile.role}</div>
            </div>
          </div>
            ...
            {nav.map((item) => {
              ...
              return (
                <Link
                  key={item.href}
                  href={item.href}
                  className="group flex min-w-0 items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-bold text-slate-700 hover:bg-slate-100 hover:text-slate-950 max-lg:min-w-[150px]"
                >
                  <Icon size={17} className="shrink-0 text-slate-400 group-hover:text-slate-700" />
      ...
      <main className="min-w-0">
        <header className="border-b border-[var(--line)] bg-white/80 px-6 py-4 backdrop-blur max-md:px-4">
