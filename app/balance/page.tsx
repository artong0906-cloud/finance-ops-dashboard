  ...
  return (
    <AppShell title="자산·자본·부채 현황" description="기초값을 입력하고 당월 증가/감소만 반영하여 기말잔액과 자본을 계산합니다.">
      <section className="grid grid-cols-3 gap-4 max-md:grid-cols-1 mb-6">
        <div className="card"><div className="text-xs font-black text-slate-500">총자산</div><div className="text-3xl font-black mt-3">{formatKRW(totalAssets)}</div></div>
        <div className="card"><div className="text-xs font-black text-slate-500">총부채</div><div className="text-3xl font-black mt-3">{formatKRW(totalLiabilities)}</div></div>
        <div className="card"><div className="text-xs font-black text-slate-500">자본</div><div className="text-3xl font-black mt-3">{formatKRW(equity)}</div><div className="text-xs text-slate-500 mt-2">총자산 - 총부채</div></div>
      </section>
      <section className="card mb-6">
        <h2 className="text-xl font-black tracking-[-0.04em] mb-4">증감 입력표</h2>
