    ...
    <AppShell title="지출 분석" description="인투1~6, 급여, 광고비, 세금, 운영비, 플랫폼 비용을 사업부별로 분석합니다.">
      <section className="grid grid-cols-6 gap-3 max-xl:grid-cols-3 max-md:grid-cols-1 mb-6">
        {["인투1 집", "인투2 차", "인투3 밥", "인투4 돈", "인투5 성장", "인투6 환경"].map((label) => {
          ...
          return <div className="card" key={label}><div className="text-xs font-black text-slate-500">{label}</div><div className="text-xl font-black mt-2">{formatKRW(total)}</div><div className="text-xs text-slate-500 mt-2">클릭 상세 예정</div></div>;
        })}
      </section>
      <section className="card">
        <h2 className="text-xl font-black tracking-[-0.04em] mb-4">지출 상세</h2>
