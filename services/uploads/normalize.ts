...
function pickContext(row: Record<string, string>, keys: string[]) {
  const values = keys
    .map((key) => pick(row, [key]))
...
export function normalizeUploadRows(uploadType: UploadType, rows: Record<string, string>[], userMappingRules: UserMappingRule[] = []): NormalizedUploadRow[] {
  return rows.map((row, index) => {
    ...
    const bankClassificationContext = uploadType === "bank" ? pickContext(row, bankClassificationContextKeys) : "";
